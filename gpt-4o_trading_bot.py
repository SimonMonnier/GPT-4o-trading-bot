...
def create_ui():
    ...
    # Convertir l'icone ICO en image via PIL
    # Si cela pose problème, fournir une image PNG.
    icon_img = None
    if os.path.exists('BTCUSD.ico'):
        ...
        # Si l'icone n'existe pas, on utilisera juste du texte
        icon_photo = None

    # Style moderne noir et bleu néon
    style = ttk.Style()
    style.theme_use('clam')
    style.configure('.', background='black')
    style.configure('TLabel', background='black', foreground='#00FFFF', font=('Arial', 12, 'bold'))
    style.configure('TButton', background='black', foreground='#00FFFF', font=('Arial', 11, 'bold'), borderwidth=1)
    style.map('TButton',
    ...
    title_label = tk.Label(header_frame, text="GPT 4o Trading Bot BTCUSD", bg='black', fg='#00FFFF', font=('Arial', 16, 'bold'))
    title_label.pack(side='left', padx=10)

    main_frame = ttk.Frame(root, style='TFrame')
    main_frame.pack(padx=10, pady=10, fill='both', expand=True)

    label = ttk.Label(main_frame, text="Clé API OpenAI :")
    label.pack(pady=5)

    api_key_var = tk.StringVar()
    api_entry = ttk.Entry(main_frame, textvariable=api_key_var, width=50, foreground='#00FFFF', background='black')
    ...
    text_widget = tk.Text(text_frame, wrap='word', bg='black', fg='#00FFFF', state='disabled', font=('Consolas', 10))
...
# ---------------------------------------------------------------------
# Code d'origine (non modifié) simplement encapsulé dans run_bot():
# ---------------------------------------------------------------------

def run_bot():
    global stop_flag, balance, trade_history
    openai.api_key = ""  # Remplacez par votre clé API
